...
// 管理用：グループ別詳細取得
settings.get('/admin/all', async (c) => {
...
// 管理用：単一設定更新
settings.put('/admin/:key', async (c) => {
...
// 管理用：複数設定一括更新
settings.put('/admin/bulk/update', async (c) => {
...
settings.get('/lp-sections/admin', async (c) => {
  const { results } = await c.env.DB.prepare(`
...
settings.put('/lp-sections/admin/:key', async (c) => {
  const key = c.req.param('key')
...
// 管理：FAQ一覧
settings.get('/faqs/admin', async (c) => {
  const { results } = await c.env.DB.prepare(`
...
// 管理：FAQ作成
settings.post('/faqs/admin', async (c) => {
...
// 管理：FAQ更新
settings.put('/faqs/admin/:id', async (c) => {
...
// 管理：FAQ削除
settings.delete('/faqs/admin/:id', async (c) => {
...
// 管理：お知らせ一覧（全件）
settings.get('/announcements/admin', async (c) => {
  const { results } = await c.env.DB.prepare(`
...
// 管理：お知らせ作成
settings.post('/announcements/admin', async (c) => {
...
// 管理：お知らせ更新
settings.put('/announcements/admin/:id', async (c) => {
  const id = c.req.param('id')
  const { title, body, type, link_url, link_text, is_visible, starts_at, ends_at, display_order } = await c.req.json()
  await c.env.DB.prepare(`
...
// 管理：お知らせ削除
settings.delete('/announcements/admin/:id', async (c) => {
